...
router_4.sql_dict[
    "3"] = "select EmployeeID id, LastName last_name, FirstName first_name, City city from Employees e order by {} limit {} offset {}"


####
def extract_end_point(request: Request):
...
def extract_sql_from_endpoint(request: Request):
    end_point = extract_end_point(request)
...
router_4.sql_dict["categories"] = \
    "SELECT	CategoryID id,CategoryName name FROM Categories order by id;"
router_4.sql_dict["customers"] = \
    'SELECT CustomerID id, CompanyName name, Address || " " || PostalCode || " " || City || " " || Country full_address FROM Customers order by id'
...
def main(limit: int, offset: int, order: str):
